...
export default function ProfilePage() {
  ...
  return (
    <div className="flex-1 flex flex-col px-4 py-5 select-none relative z-10 gap-5 pb-20">
      ...
      <div className="glass-panel p-5 rounded-3xl border border-white/10 relative overflow-hidden flex flex-col items-center gap-3 bg-gradient-to-b from-white/5 to-transparent">
        ...
        <div className="w-16 h-16 rounded-full border-2 border-brand-level p-1 bg-black relative z-10 shadow-[0_0_15px_rgba(157,78,221,0.3)]">
        ...
        <div className="text-center relative z-10">
          ...
          <span className="text-[9px] bg-white/5 border border-white/5 text-zinc-500 px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider mt-2.5 inline-block">
            {profile.currentStage}
          </span>
        </div>

        {/* XP Ledger Info */}
        <div className="w-full grid grid-cols-3 gap-2 mt-2 pt-3 border-t border-white/5 text-center relative z-10">
          <div>
            <span className="text-[8px] uppercase font-bold text-zinc-500 tracking-wider">Level</span>
            <span className="text-sm font-black text-white block mt-0.5">{profile.level}</span>
          </div>
          ...
          <div>
            <span className="text-[8px] uppercase font-bold text-zinc-500 tracking-wider">Streak</span>
        ...
        {(['DNA', 'Timeline', 'Logs'] as const).map((tab) => {
          ...
          return (
            <button
              ...
              className={`py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all duration-200 ${
                isSelected
                  ? 'bg-white/10 text-white shadow-inner font-extrabold border border-white/5'
      ...
      <div className="flex-1 flex flex-col relative z-10">
        ...
        {activeTab === 'DNA' && (
          ...
          >
            {/* DNA Orb Badge */}
            <div className="glass-panel rounded-3xl p-4 border border-white/5 flex flex-col items-center">
        ...
        {activeTab === 'Timeline' && (
          ...
          >
            <div className="relative border-l border-dashed border-zinc-700/60 flex flex-col gap-6 pl-5 py-2 select-none text-left">
              {profile.timeline.map((milestone, idx) => (
                <div key={idx} className="relative">
                  {/* Glowing vertical node */}
                  <span className="absolute -left-[27px] top-0.5 w-3 h-3 rounded-full bg-brand-level border-2 border-black shadow-[0_0_8px_#9d4edd] animate-pulse" />
          ...
          >
            ...
            {profile.logs.length === 0 ? (
              ...
              <div className="glass-panel p-6 rounded-3xl border border-white/5 text-center flex flex-col items-center gap-3">
                ...
                <button
                  onClick={() => router.push('/home')}
                  className="px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-white text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 transition-colors duration-150"
                >
              ...
              profile.logs.map((log) => (
                <div key={log.id} className="glass-panel p-4 rounded-2xl border border-white/5 flex flex-col gap-2 select-none text-left">
                  ...
                  <p className="text-xs text-zinc-200 font-semibold leading-relaxed mt-1">
                    {log.content}
                  </p>

                  <div className="flex items-center gap-2 border-t border-white/5 pt-2.5 mt-1 text-[9px] text-zinc-500 font-bold uppercase tracking-wider">
